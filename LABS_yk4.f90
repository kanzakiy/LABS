   ...
   write(Today,*) "_2_Times_resolution_"

   ! Begin initialisations

       ! OPEN(unit = File_Profile,  file = 'DepthProfiles.OUT', status = 'unknown')
       ! OPEN(unit = File_Scaling, file = 'PorosityScale.OUT', status = 'unknown')
       ! OPEN(unit = File_Activity, file = 'ActivitySlope.OUT', status = 'unknown')
       ! OPEN(unit = File_Displace, file = 'MeanDisplacement.OUT', status = 'unknown')

       OPEN(unit = File_Profile,  file = 'C:/Users/User/Desktop/biot-res/DepthProfiles'          &
                               //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Profile2,  file = 'C:/Users/User/Desktop/biot-res/DepthProfiles2'        &
                               //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Profile3,  file = 'C:/Users/User/Desktop/biot-res/DepthProfiles3'        &
                              //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Profile_Re,  file = 'C:/Users/User/Desktop/biot-res/DepthProfiles_Re'        &
                             //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Scaling, file = 'C:/Users/User/Desktop/biot-res/PorosityScale'        &
                              //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Activity, file = 'C:/Users/User/Desktop/biot-res/ActivitySlope'        &
                               //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Displace, file = 'C:/Users/User/Desktop/biot-res/MeanDisplacement'        &
                               //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = File_Displace2, file = 'C:/Users/User/Desktop/biot-res/MeanDisplacement2'        &
                               //trim(adjustl(today))//'.OUT', status = 'unknown')
       OPEN(unit = Poly_fit, file = 'C:/Users/User/Desktop/biot-res/polyFit'                           &
                                  //trim(adjustl(today))//'.OUT', status = 'unknown')
   ...
   Savetime = 1 !  added ----- YK 5/22/2017

       DO Time = 1, TimeMax

         ! calculate some parameters for this time step:
         !   seasonal and Diurnal variations in activity, time string
         !   Pr_Activity_year = annual cycles (t=0 is "winter" .. least active period)
         !   Pr_Activity_day  = daily cycles  (t=0 is "midnight" .. least active period)
   ...
   ! clean up data allocations
       ...
       Deallocate (Org, Matrix, Particle, Org_Loc, Guts, MovementHistory, IngestionHistory)
   ...
   ! be (MinHeadWidth) times smaller than the smallest head width (PixelSize)
       ...
       write(*,*) "ParticleSIze, PixelSize:", ParticleSIze, PixelSize
   ...
   ! The times for data output .. make this a user input, eventually, via a namelist
   !  N_Outputs = 5
      N_Outputs = 15
      N_Outputs2 = 100000
  ...
  !  Time_Output =(/1,100*Day,1*Year,5*Year,100*Year/)
   ...
   ! rescale time to units of simulation
       ...
       TimeStep = r_TimeStep / TimeScale + Int(2.*mod(r_TimeStep,Timescale))
         ...
         print *, "r_TimeStep(day), Timestep, Timescale", r_TimeStep, Timestep, Timescale
         print *, "r_TimeMin(day), TimeMin, Timescale", r_TimeMin, TimeMin, Timescale
         print *, "r_TimeMax(day), TimeMax, Timescale", r_TimeMax, TimeMax, Timescale
   ...
   ! transition probabilities of lability change (1) == 1 to 0, (2) == 2 to 1, etc. .. as halflife in year**(-1)
       ...
       Lability_decayConstant(1) = -99999              ! cannot get smaller .. null value
       Lability_decayConstant(2) = -log(0.5) / 100     ! i.e., ln(0.5)/halflife(in days)
       Lability_decayConstant(3) = -log(0.5) / 10
       Lability_decayConstant(4) = -log(0.5) / 1
       Lability_decayConstant(5) = -log(0.5) / .1

   ! the relative abundance of the various lability classes of newly sedimenting particles
   ...
   ! MovementHistory and IngestionHistory contain information about past rates of movement and ingestion ..
   ! EOSHIFT shifts the elements of the matrix and results in a moving average with a window of set size
...
!!!  YK   initiall run should underestimate average values because the present values are not reflected ?       
...
! dead .. remove and start a new organism
   ...
   ! As the organism is about to move .. check for exceptions/boundaries
       ...
       j = Int(URand(1) * Real(Org(i)%Width)) + 1   ! YK   why random number?
       Org(i)%Is%OnTheEdge = .false.
       ...
       Select Case (Direction(j))
         ...
         Do Y = (Head_MinVal%Y-1), Head_MinVal%Y-Org(i)%SensoryRange, -1
           ...
           Do X = (Head_MinVal%X-index), (Head_MaxVal%X+index), 1
             Call TabulateInfo(Y, X, j, Lability_sum, N_Particles, N_Water, N_Organism)
           End do
         End do
   ...
   SUBROUTINE TabulateInfo(Y, X, j, Lability_sum, N_Particles, N_Water, N_Organism)
   ...
   Integer     :: Lability_New      !YK  function  
       ...
       Case(p)
         N_Particles(j) = N_Particles(j) + 1
           IF (Lability_New(Matrix(Y,X)%Value) .le. 0) then
             Lability_sum(j) = Lability_sum(j) + 0.1   ! a small positive real value /...
           Else
   ...
   Select Case  (Angle)
     Case (-90)            ! rotate the head left (counter-clockwise) 90 degrees
       do j = 1, Org(i)%HeadSize
         Org_Loc(j,i) = Coordinates(-(Image(j)%X-Head_MinVal%X)+Head_MinVal%Y+(Org(I)%Width-1), &
                                   Image(j)%Y-Head_MinVal%Y+Head_MinVal%X)       
                           ! YK   rotating around (head_minX, head_minY ) and translation in y-axis by orgWidth,
                           ! YK   so that there is no net movement of org
     ...
     Case (90)             ! rotate the head right (clockwise) 90 degrees
       do j = 1, Org(i)%HeadSize
         Org_Loc(j,i) = Coordinates((Image(j)%X-Head_MinVal%X)+Head_MinVal%Y,      &
                              - (Image(j)%Y-Head_MinVal%Y)+Head_MinVal%X+(Org(I)%Width-1))     
                           ! YK   rotating around (head_minX, head_minY ), and translation in x-axis by orgWidth
         Matrix(Org_Loc(j,i)%Y, Org_Loc(j,i)%X) = Image_ID(j)
       end do
...
! ****************************************************************
  ...
  ! ********************************
   ...
   End if
   ! YK    Is the following repeating the same thing twice if the above if is the case?
       Call Mouth_locate(i, Point, Org(i)%Orientation)
       Call Particles_ingest(i, Point, Org(i)%Orientation)
       Call Particles_push(i, Point, Org(i)%Orientation)
...
ChooseDir:  Do While (Sum(DirWeights) .ne. 0) ! else return to the main do loop
     ...
     DO k = 1, Org(I)%Width
       ...
       EAT = .FALSE.
         If (Lability_ON) then                                 ! lability searching function is on
           ...
           If (n .ge. 3) Eat = .true.                          ! lability threshold: < class 2 particles are rejected
               ...
               Particle(Guts(j,i)%Value)%loc = Tail(loc)
                 If (Lability_ON) then       ! lability functions are on ..
                   IF (Particle(Guts(j,i)%Value)%Lability .gt. 1) then
                     Particle(Guts(j,i)%Value)%Lability = Particle(Guts(j,i)%Value)%Lability - 1
   ...
   Call Output_Graphic()
     ...
     Call Output_txtImg() ! uncomment if ascii data desired
     Call Output_O2txtImg() ! uncomment if ascii data desired
   ...
   ! AsciiFile2 = 
   OPEN(unit = File_ASCII, File = 'C:/Users/User/Desktop/biot-res/data'//trim(adjustl(today))    &
                //trim(adjustl(numtemp))//'.ascii', status = 'unknown') 
   OPEN(unit = File_txtImg_2, File = 'C:/Users/User/Desktop/biot-res/data_2'//trim(adjustl(today))    &
                //trim(adjustl(numtemp))//'.txt', status = 'unknown')
     DO X = 1, N_col
     DO Y = 1, N_row
       If (IsParticle(CoOrdinates(Y,X))) then
         Code = 10
         ! WRITE(File_txtImg_2,*) X, Y, 1, 1. !, Matrix(Y,X)
       Else if (IsWater(CoOrdinates(Y,X))) then
         Code = 0
         WRITE(File_txtImg_2,*) X, Y, 1  !, 0. !, Matrix(Y,X)
   ...
   write(numtemp,'(i10.1)') SaveTime
   ! AsciiFile2 = 
   OPEN(unit = File_txtImg, File = 'C:/Users/User/Desktop/biot-res/data'//trim(adjustl(today))          &
                        //trim(adjustl(numtemp))//'.txt', status = 'unknown')
     DO Y = 1, N_row
     txtimg = 0 
     DO X = 1, N_col
   ...
   ! AsciiFile2 = 
   OPEN(unit = File_txtImg, File = 'C:/Users/User/Desktop/biot-res/o2/O2data'//trim(adjustl(today))          &
                        //trim(adjustl(numtemp))//'.txt', status = 'unknown')
     DO Y = 1, N_row
     txtimg = 0 
     DO X = 1, N_col
   ...
   integer :: x, y
   real :: po
   real :: po_particle = 0.85  !  porosity of particle
   ...
   do y = 1, n_row
     ...
     ! write(*,*) (Matrix(y,i)%Class, i = 1, n_col)
     
     do x = 1, N_Col
          
     O2(Y,X)%Oxygen_pre = O2(y,x)%Oxygen
     O2(Y,X)%Value_pre = Matrix(y,x)%Class
   ...
   ! integer, parameter :: n = (N_Row - 1)*N_Col
   ! integer , parameter :: nnz = 5*(n - 2*N_Col) + 4*2*N_Col 
   ...
   integer :: y_int        !  upper y layer where oxygen = iox
   integer :: n_row_calc       !  number of row used for the calculation
   integer :: y_fin       !  bottom y layer
   
   real :: iox = 1.0    ! initial conc of oxygen (mol/L)
   real :: dif_0 = 549. ! cm^2/yr
   real :: po_particle = 0.5  !  porosity of particle
   ! real :: po_particle = 1.  !  porosity of particle
   integer :: j, k, cnt, cnt2, xx, yy
   logical :: matrix_chk
   ...
   if (only_sed) then 
   y_int  = N_RowWater + 1      ! water/sediment interface is boundary 
   n_row_calc = N_rowSed
   y_fin = n_row   
   ...
   n = (N_row_calc - 1)*N_col
   nnz = 5*(n - 2 * N_col) + 2*4*N_col
   
   ! DeAllocate (ai, ap, ax, b, x)
   Allocate (ai(nnz),ap(n+1),ax(nnz),b(n),x(n))
   
   ! write(*,*) n_row,n_col,n,nnz
   ...
   do yy = y_int  + 1, y_fin
     ...
     do xx = 1, n_col
       ...
       j = xx + (yy - y_int  - 1) * n_col    ! matrix number
       cnt = 0
       
       if (matrix(yy,xx)%Class /= p) po_c = 1.
       if (matrix(yy,xx)%Class == p) po_c = po_particle
       
       if (O2(yy,xx)%Value_pre /= p) po_pre = 1.
       if (O2(yy,xx)%Value_pre == p) po_pre = po_particle
       ! po_c = 1.*(matrix(yy,xx)%Value /= 0) + (matrix(yy,xx)%Value == 0)*0.85
       ox_c = O2(yy,xx)%Oxygen_pre * po_pre
       
       dif_c = dif_0 * po_c * po_c   ! Aller (1980)
       
       rxn = O2(yy,xx)%oxygen_use
       
       b(j) = b(j) - ox_c/dt + rxn
     
       if (yy == y_int  + 1) then
           
         if (matrix(yy - 1,xx )%Class /= p) po_u = 1.
         if (matrix(yy - 1,xx )%Class == p) po_u = po_particle
         dif_u = dif_0 * po_u * po_u   ! Aller (1980)
           
         if (matrix(yy + 1,xx )%Class /= p) po_d = 1.
         if (matrix(yy + 1,xx )%Class == p) po_d = po_particle
         dif_d = dif_0 * po_d * po_d   ! Aller (1980)
       
         b(j) = b(j) -dif_c*iox/dx/dx - (dif_c - dif_u)/dx/dx*iox*(-1.0)   !  upper boundary condition
         
         if (xx == 1) then
       
           if (matrix(yy,xx + n_col - 1)%Class /= p) po_l = 1.
           if (matrix(yy,xx + n_col - 1)%Class == p) po_l = po_particle
           dif_l = dif_0 * po_l * po_l   ! Aller (1980)
           
           if (matrix(yy,xx + 1)%Class /= p) po_r = 1.
           if (matrix(yy,xx + 1)%Class == p) po_r = po_particle
           dif_r = dif_0 * po_r * po_r   ! Aller (1980)
           
         
           k = j
           ax(cnt2) = -dif_c*4./dx/dx - 1./dt + (dif_c - dif_l)/dx/dx + (dif_c - dif_u)/dx/dx
           ai(cnt2) = k 
           cnt2 = cnt2 + 1
           cnt = cnt + 1
           
           k = j + 1
           ax(cnt2) =  dif_r/dx/dx + (dif_r - dif_c)/dx/dx*(-1.)
           ai(cnt2) = k 
           cnt2 = cnt2 + 1
           cnt = cnt + 1
           ...
           k = j + n_col   
           ax(cnt2) =  dif_d/dx/dx  + (dif_d - dif_c)/dx/dx*(-1.0) 
           ai(cnt2) = k 
           cnt2 = cnt2 + 1  
           cnt = cnt + 1  
           ...
           if (matrix(yy,xx - 1)%Class /= p) po_l = 1.
           if (matrix(yy,xx - 1)%Class == p) po_l = po_particle
           dif_l = dif_0 * po_l * po_l   ! Aller (1980)
           
           if (matrix(yy,xx -n_col + 1)%Class /= p) po_r = 1.
           if (matrix(yy,xx -n_col + 1)%Class == p) po_r = po_particle
           dif_r = dif_0 * po_r * po_r   ! Aller (1980)
           ...
           k = j   
           ax(cnt2) =  -dif_c*4./dx/dx  + (dif_c - dif_l)/dx/dx + (dif_c - dif_u)/dx/dx - 1./dt
           ai(cnt2) = k 
           cnt2 = cnt2 + 1   
           cnt = cnt + 1   
           
           k = j + n_col   
           ax(cnt2) =  dif_d/dx/dx + (dif_d - dif_c)/dx/dx*(-1.)  
           ai(cnt2) = k 
           cnt2 = cnt2 + 1       
           cnt = cnt + 1       
           ...
           k = j  + 1
           ax(cnt2) =  dif_r/dx/dx + (dif_r - dif_c)/dx/dx*(-1.0) 
           ai(cnt2) = k 
           cnt2 = cnt2 + 1   
           cnt = cnt + 1   
         ...
         if (xx == 1) then
           ...
           k = j 
           ax(cnt2) =  dif_c * (-3.0)/dx/dx + (dif_c - dif_l)/dx/dx + (dif_c - dif_u)/dx/dx -1./dt
           ai(cnt2) = k 
           cnt2 = cnt2 + 1
           cnt = cnt + 1
           ...
           k = j  
           ax(cnt2) =  dif_c * (-3.)/dx/dx + (dif_c - dif_l)/dx/dx + (dif_c - dif_u)/dx/dx - 1./dt
           ai(cnt2) = k 
           cnt2 = cnt2 + 1   
           cnt = cnt + 1   
         ...
         if (xx == 1) then
           ...
           k = j 
           ax(cnt2) =  dif_c * (-4.0)/dx/dx + (dif_c - dif_l)/dx/dx + (dif_c - dif_u)/dx/dx - 1./dt
           ai(cnt2) = k 
           cnt2 = cnt2 + 1
           cnt = cnt + 1
...
!
  ...
  do yy = 1, n_row
   ...
   Integer :: lab_rowsum(N_LabilityClasses), Particle_Lability, Lability_New, P_count
   ...
   ! DO SOME INITIAL DATA MANIPULTAIONS AND CALCULATIONS
     ...
     DO Y = 1, n_row   ! INTEGRATE ACROSS
       ...
       DO X = 1, N_Col
         ...
         If (IsParticle(CoOrdinates(Y,X))) then
           ...
           Particle_Lability = Lability_New(Particle_ID)
             ...
             If (Particle_Lability .gt. 0) lab_rowsum(Particle_Lability) = lab_rowsum(Particle_Lability) + 1
       ...
       ! set up data vectors for linear regression analysis
         ...
         WRITE(File_Profile,*) Timevalue, Depth, Activity_mean, log10(Activity_mean), por_mean, lab_rowsum, MatrixOccupancy(Y)
         ...
         Do k = 2, N_LabilityClasses
           If (lab_rowsum(k) .le. 1) then  ! remove small numbers and zero values from analysis
             ! Lability_vector(y,k) = AMACH(6)  ! NaN .. not a number in the IMSL libraries
    ...
    ! do the linear regression estimate of the slope FOR THE TRACER
       ...
       if (cnt-1/=m) print *, "something", cnt, m

         If (m .gt. 5) then     ! if enough data ..
           ! CALL RONE (N_Row,3,Profile,N_Row,1,2,1,0,3,0,95.,95.,0,AOV,COEF,2,COVB,2,TESTLF,CASE,N_Row,NRMISS)
             ! If (Coef(2,1) .NE. 0) then   ! if slope ne 0
               ! Db = DecayConstant/(Coef(2,1)*Coef(2,1))
               
          !!  Try to calculate in a easy way----YK, 5/22/2017     
           ! avx = sum(Profile(:,1)*Profile(:,3))/m
           ! avy = sum(Profile(:,2)*Profile(:,3))/m
           ! sumx2 = sum((Profile(:,1)*Profile(:,3)-avx)**2.0)
           ! sumy2 = sum((Profile(:,2)*Profile(:,3)-avy)**2.0)
           ! sumxy = sum((Profile(:,2)*Profile(:,3)-avy)*(Profile(:,1)*Profile(:,3)-avx))
           ! avx = sum(Profile(:,1))/m
           ! avy = sum(Profile(:,2))/m
           ...
           slp = sumxy/sumx2
           intcpt = avy - slp*avx              ! Is this correct?----YK, 5/22/2017
           crr = sumxy**2.0d0/sumx2/sumy2     ! Is this correct?----YK, 5/22/2017
           ...
           Db = DecayConstant/(slp*slp)   !  cm^2 yr^-1
            ...
            ! output Db, Error of Db , slope, SE Slope ,intercept, SE intercept, R2 , p-value, error df
            ! SEE IMSL DOCUMENTATION FOR MORE DETAILS (OR BEGINNING OF THIS SUBROUTINE, ABOVE)
               ! WRITE (File_Activity,30) TimeValue, ' 210Pb', Db, abs(Db*Coef(2,2)/Coef(2,1)),   &
                       ! Coef(2,1), Coef(2,2), Coef(1,1), Coef(1,2), AOV(11), AOV(10), AOV(2)
             End if
         End if
         ...
         End do
          ...
          !!  Try to calculate in a easy way----YK, 5/22/2017     
           ...
           Db_lability = Lability_decayConstant(n)*365.0/(slp*slp)      !! cm^2/yr
               ...
               ! output Db, Error of Db , slope, SE Slope ,intercept, SE intercept, R2 , p-value, error df
               ! SEE IMSL DOCUMENTATION FOR MORE DETAILS (OR BEGINNING OF THIS SUBROUTINE, ABOVE)
                 ! WRITE (File_Activity,31) TimeValue, n, Db_Lability, abs(Db_Lability*Coef(2,2)/Coef(2,1)),    &
                                   ! Coef(2,1), Coef(2,2), Coef(1,1), Coef(1,2), AOV(11), AOV(10), AOV(2)
               End if
           End if
       End do
   ...
   ! OUTPUT THE PARTICLE DEVIATIONS
     ...
     do y = 1, n_row
       ...
       ! Db_list(y) = merge((PixelSize * RMS2(y)**2.0*365./time/timescale/4.),0.0/RMS2(y),(RMS2(y).ne.0).and.(Time.ne.0))
       Db_list(y) = RMS2(y)*365./time/timescale/4.
       ! Db_list(y) = 0.1
       
       print *, y,(Db_list(y)),(.not.ieee_is_nan(Db_list(y))).and. (y .le. n_row).and.(Db_list(y).ne.0)
       
       write(File_Displace2,*) Timevalue, Depth_vector(y), DEV_Y2(y), DEV_X2(y), RMS2(y),   &
           merge(log10(Db_list(y)),0.0/RMS2(y),.not.ieee_is_nan(Db_list(y)) .and.(Db_list(y).ne.0.)), P_COUNT2(y)
     ...
     cnt = 0
       do j = 1, N_Row
         ! if ( IfNan(Profile(j,1)) .or. IfNAN(Profile(j,2)) ) m = m-1
         ! if (ieee_is_nan(Db_list(j)) .or.(Db_list(j).eq.0.)) m = m-1
         if (.not.ieee_is_nan(Db_list(j)) .and.(Db_list(j).ne.0.)) cnt = cnt +1
       End do
        m = cnt
    ...
    if (m > max(5,poly_fit_n)) then
     ...
     do j = 1, N_row
        if (cnt.eq. m) exit
        if (.not.ieee_is_nan(Db_list(j)) .and.(Db_list(j).ne.0.)) then
        print *,cnt, Db_list(j), log10(Db_list(j))
        
        xsel(cnt) = depth_vector(j)
        ysel(cnt) = log10(Db_list(j))
        sig(cnt) = 1.0 !abs(log10(Db_list(j)) - log10(Db))
        
        cnt = cnt + 1
        end if
     end do
     
     if (cnt.ne.m) print *, "!!error!!", cnt, m
           
           call pfit(dble(xsel),dble(ysel),sig,a,coeff,cov)
           
           print*,(a(j), j = 1, poly_fit_n)
    ...
    do y = 1, n_row
       if (y .ge. yfin) then 
         ...
         do while (((.not.ieee_is_nan(Db_list(j))).and. (j .le. n_row).and.(Db_list(j).ne.0))) 
           if (cnt2 .eq. 0) then 
             yint = j
             ! print *, "cnt2 ==0, yint, cnt2 ",  yint , cnt2
           end if
           cnt2 = cnt2 + 1
           j = j + 1
           ...
           ! print *, "j, cnt2 ",j,  cnt2, Db_list(j), ((.not.ieee_is_nan(Db_list(j))).and. (j .le. n_row).and.(Db_list(j).ne.0))
           ! end if 
         end do
       end if
     ...
     if (maxval(list(3,:)) > 50) then
        print *, "time_to_calculalte_activity",time
        print *, maxloc(list(3,:)), list(3,maxloc(list(3,:)))
        cnt2 = maxval(list(3,:)) 
        yint = maxval(list(1,maxloc(list(3,:))))
        yfin = maxval(list(2,maxloc(list(3,:))))
        print *, "yint, yfin, cnt2, ",  yint, yfin, cnt2
        ...
        ! do y = list(1,maxloc(list(3,:))) + 1, list(2,maxloc(list(3,:)))
        do y = yint , yfin 
          row = y -  yint + 1
          print *, row, cnt2,  Db_list(y)
          ! if (y ==   list(1,maxloc(list(3,:))) + 1) then
          if (y ==   yint ) then
            ...
            ymx(row) = (Db_list(y) - Db_list(y))/pixelSize/pixelSize* merge(Activity_vector2(y-1),1.      &
                       , (Activity_vector2(y-1).ne.0.0).and.(.not.ieee_is_nan(Activity_vector2(y-1)))     &
                       .and.(yint.ne.1).and.(.not.int_activity_1)) &
                     +  Db_list(y)*(-1.0)/pixelSize/pixelSize * merge(Activity_vector2(y-1),1.   &
                     ,  (Activity_vector2(y-1).ne.0.0).and.(.not.ieee_is_nan(Activity_vector2(y-1)))       &
                     .and.(yint.ne.1).and.(.not.int_activity_1))
          ! else if (y == list(2,maxloc(list(3,:)))) then 
          else if (y == yfin) then 
            ...
            ! if (yfin == n_row) then 
              ! ymx(row) =  Db_list(y)*(-1.0)/pixelSize/pixelSize *activity_vector2(y)
            ! else
              ! ymx(row) =  Db_list(y)*(-1.0)/pixelSize/pixelSize *merge(activity_vector2(y+1),activity_vector2(y)       &
                    ! ,  (Activity_vector2(y+1).ne.0.0).and.(.not.ieee_is_nan(Activity_vector2(y+1))))
            ! end if
          else 
        ...
        open (unit=100, file="C:/Users/User/Desktop/biot-res/check.txt",status = 'unknown')
        do y = yint, yfin
          row = y - yint + 1
        ...
        open (unit=200, file="C:/Users/User/Desktop/biot-res/check2.txt",status = 'unknown')
        do y = yint, yfin
          row = y - yint + 1
          write(200,*) ymx(row)
        end do 
        ...
        ! call inverse(amx, amx2, cnt2)
        ! call dot(amx2, ymx, ymx2, cnt2)
        ! call dot(amx, ymx2, cmx, cnt2)
        
        ! if ((maxval(cmx - ymx) .ne. 0.) .or. (minval(cmx - ymx).ne.0.)) print*,"error",maxval(cmx - ymx),minval(cmx - ymx)
       ...
       If (OutputScaling) then
           ...
           Allocate (PorosityData(N_Samples))
             ...
             Do While (Window .lt. (N_Row)/2)
                 ...
                 ! Call UVSTA (0, N_Samples, 1, PorosityData, N_Samples, 0, 0, 0, 95.0, 95.0, 0, STAT, 15, NRMISS)
                 ! por_mean = Stat(1,1)
                 ! por_stderr   = STAT(3,1)
...
40  FORMAT (A8, 3f11.6)
   ...
   If (PlotMatrix) then
       ...
       call writegif("C:/Users/User/Desktop/biot-res/lab/lability"          &
   ...
   If (.not. Movies_On) then
    If (PlotPorosity) then
       ...
       call writegif("C:/Users/User/Desktop/biot-res/por/porosity"           &
                 //trim(adjustl(today))//trim(adjustl(numtemp))//".gif", image_data, palette)
    ...
    If (PlotActivity) then
       ...
       call writegif("C:/Users/User/Desktop/biot-res/act/activity"//            &
                   trim(adjustl(today))//trim(adjustl(numtemp))//".gif", image_data, palette)
...
!    End if
  ...
  ! *********************
   ...
   real :: Corg_int = 1.0   ! mol/L initial organic matter concentration of each particle
     ...
     Lability_new = Particle(ID)%Lability
       ...
       END IF
         ...
         Call Random_Number(URand)
           TimeIncrement = Real(Time - Particle(ID)%Lability_Time0) * TimeScale / 365.
           Pr_Cummulative = 1. - exp(-Lability_decayConstant(Lability)*365.*TimeIncrement)  
           ! YK 365. is added to make units in yr
           If (URand .lt. Pr_Cummulative) then
             
    ! matrix(coordinates(particle(ID)%loc))%Oxygen_use = Lability_decayConstant(Lability) *TimeScale / 365.
    O2(particle(ID)%loc%Y,particle(ID)%loc%X)%Oxygen_use = &
             Lability_decayConstant(Lability) *365.* exp(150.*(1.*lability/N_LabilityClasses - 1.))*Corg_int
